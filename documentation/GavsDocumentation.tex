...
% Define block styles
\tikzstyle{decision} = [diamond, draw, text centered,  inner sep=2pt, rounded corners=0pt]
\tikzstyle{block} = [rectangle, draw, text centered, rounded corners]
\tikzstyle{line} = [draw, -latex', rounded corners=10pt]
\tikzstyle{stop} = [draw, ellipse, minimum height=2em]
\tikzstyle{description} = [node distance=1mm]
...
\newcommand{\inoutentry}[7][]{% #1 (optional) emphasized index entry? #2 variable name, #3 variable type, #4 dimensions, #5 math symbol, #6 unit, #7 description
...
\bibliography{literature/GavansDocumentation}
