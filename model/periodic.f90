...
REAL*8,DIMENSION(18) :: readRealParams
...
DO i=1,k  
  ...
  READ(99,'(18f23.7)') readRealParams(:)
 ...
 !ET(r) =  beta*exp(-kc*r)*dx*dy
  ...
  kv =   readRealParams(15) !40.0d0
  Dv =   readRealParams(16) !0.1d0
  kb =   readRealParams(17) !20.0d0
  Db =  readRealParams(18) ! 0.1d0
  !i=3
  j=1
  WRITE(fid,'(i4)') i
  WRITE(fid2,'(i4)') j
...
DO i=1,m
  DO j=1,n
   ...
   flowResistance0(i,j) = (dble(rnd)*kb+1.0d0)
   CALL random_number(rnd)
   !If(i<j) THEN
   !If[i > j, 60 - ((i 1 + j 6) ), 60 - (( i 6 + j 1))]
   topog(i,j)  =0.3d0*dble(i)+0.0d0*dble(j) + 1000.d0+ dble(rnd -0.5d0)*roughness
...
flowdirns = -2
 ...
 !infiltKern = 0.1d0
 !storeKern = 99999.d0
 ! flowResistance1 = kb+1000.d0
 !DO j=1,100
 !CALL RoutingWithKernel(m,n,mn,precip, infiltKern, storeKern, flowdirns,topog, store,discharge,outflow)
 !CALL Erosion(discharge,topog,flowdirns,flowResistance1,m,n)
 ! topog(1,1)=minval( (/ topog(1,1)-0.01d0, topog(1,2)-0.01d0,topog(2,1)-0.01d0 /) )
 !CALL GD8(topog,flowdirns, m,n)
 !PRINT *, topog
 !store = 0
 !END DO     
...
DO i=1,m
  DO j=1,n
    if (veg(i,j)>0) THEN
       flowResistance1(i,j) = flowResistance0(i,j)+kv
    ELSE
        flowResistance1(i,j)= flowResistance0(i,j)
   END IF
END DO
END DO
...
DO j=1,nSteps
 ...
 !END IF
  ...
  !DO k=1,1  !erosion, routing, evaporation, loop
    ...
    IF ((simflags(1).eq.1).and.(simflags(2).eq.1).and.(j>20)) THEN
      CALL Erosion(discharge,topog,flowdirns,flowResistance1,m,n)
      !topog(1,1)=topog(1,1)-0.005d0
      DO k=1,10
...
END IF  
  ...
  IF ((simflags(3).eq.1).and.(flag.eq.1)) THEN
    ...
    veg = veg + dummyveg  !add on emerging vegetation
     ...
     DO k=1,n
      if (veg(i,k)>0) THEN
        flowResistance1(i,k) = flowResistance0(i,k)+kv
      ELSE
        flowResistance1(i,k)= flowResistance0(i,k)
      END IF
    END DO
    END DO
  END IF
...
SUBROUTINE Erosion(discharge,topog,newflowdirns,flowResistance,m,n)
  IMPLICIT NONE
  Integer, INTENT(IN) :: m,n
  Integer, DIMENSION(m,n), INTENT(IN) :: discharge, newflowdirns  !dischange has units of mm/year
  REAL*8, DIMENSION(m,n), INTENT(IN) :: flowResistance !effective d40 grainsize in mm
  ...
  !Parameters
  timestep = 12.d0*24.0d0*60.d0*60.d0  !  (* effective flow duration in seconds per year*)
  time = timestep
  bulkdensity = 2650.d0 ! (* (kg/m^3) *) 
  porosity = 0.4d0  !(-)
  cellLength = 1.d0 !(m)
  ...
  flow = 4.d0*DBLE(discharge)*(cellLength)**2/(1000.d0)/(365.d0*24.d0*60.d0*60.d0) !  conversion of mm/cell/year to m^3/sec
  
  Qs_in = 0.d0  ! (* sediment flux in kg/s*)
  Qs_out = 0.d0 !sediment flux out kg/s
  dx = cellLength !flow length, dx (m)
...
SUBROUTINE Splash(topog,veg, Dv, Db,m,n)
  ...
  cellLength = 1.0d0
  deltaz = 0.0d0
  deltat = (1.0e-1)*30.d0/365.d0 !kyr
