...
\begin{tikzpicture}[node distance = 5mm, auto]
    ...
    \node [block, below=of start, node distance = 1cm] (read) {read input};
    
    \node [decision, below=of read, node distance = 1.5cm] (run) {run?};
    \node [block, below=of run] (derive) {derive input parameters};
    \node [decision, below=of derive] (useRandomSeed) {use random seed?};
    ...
    \node [decision, below=of  SimCode, node distance=2.3cm] (repeat) {is there another parameter set?};
    \node [stop, right of=repeat, node distance=4cm] (stop) {stop};
    \path [line] (start) -- (read);
    \path [line] (read) -- (run);
    \path [line] (run) -- node {yes} (derive); % node [near start] {yes}
    \path [line] (derive) -- (useRandomSeed);
    \path [line] (useRandomSeed) -- node {yes} (initRandomSeed);
    \path [line] (useRandomSeed) -- node {no} (SimCode);
    \path [line] (initRandomSeed) |- (SimCode);
    \path [line] (SimCode) -- (repeat);
    \path [line] (repeat) -- node {no} (stop);
    \path [line] (run) -| node [near start] {no} (stop);
    \path [line] (repeat.west) -| ++(-.5, 0) |- node [near start, right] {yes} (read.west);
\end{tikzpicture}
...
\begin{figure}
\begin{center}
\begin{tikzpicture}[node distance=4mm, align=center, every path/.style={line}] %every join/.style={-latex'}
    \matrix[row sep=4mm,column sep=8mm] {
        ...
        &\node (zero) [block] {zero actual ET,\\ bare E and outflow}; & \\
        &\node (usetopo) [decision] {use\\ topography to route flow?}; &    \node (rwk) [block] {route with kernel}; \node (simErosion) [decision, below=of rwk] {simulate\\ erosion?};
        \node (erosion) [block, right=8mm of simErosion, ] {erosion}; 
        \node (splash) [block, below=of erosion] {splash};
        \node (GD8) [block, below=of splash] {GD8};\\
        &\node (evap) [decision] {simulate with eva-poration?};  & \node (evaporation) [block] {evaporation};\node (vegChange) [block, below=of evaporation] {vegetation change};\\
        ...
        \node (changeInfilt) [block, below=of vegEvolve] {change infiltration \\ propterties}; 
